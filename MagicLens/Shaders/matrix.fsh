...
void main() {
    ...
    vec2 uv = vec2(1.0 - position.y, 1.0 - position.x);
    uv.y *= (u_Resolution.x*u_CameraResolution.x)/(u_CameraResolution.y*u_Resolution.y);

    float globalTime = u_GlobalTime * RAIN_SPEED;
    ...
    result = result * length(texture2D(u_VideoFrame,uv).rgb) + 0.22 * vec4(0.,texture2D(u_VideoFrame,uv).g,0.,1.);
    if(result.b < 0.5)
