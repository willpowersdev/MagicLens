...
fragment float4 fragment_edgehighlights(VertexOut interpolated [[stage_in]],
    ...
    float4 texColor = sampleVideo(video, uv);
    float2 texStep = 1.0 / uniforms.resolution;

    float4 s01 = sampleVideo(video, uv - float2(1.0, 0.0) * texStep);
    float4 s02 = sampleVideo(video, uv + float2(1.0, 0.0) * texStep);
    float4 s03 = sampleVideo(video, uv - float2(0.0, 1.0) * texStep);
    float4 s04 = sampleVideo(video, uv + float2(0.0, 1.0) * texStep);
